...
if [ -z "${API_BASE_URL:-}" ]; then
	echo "boardly-web: API_BASE_URL is required (e.g. http://<server>:8040)" >&2
	exit 1
fi
...
window.__BOARDLY__ = { apiBaseUrl: "$(js_string "$API_BASE_URL")", apiKey: "$(js_string "${API_KEY:-}")" };
CONFIG
echo "boardly-web: API at $API_BASE_URL"
