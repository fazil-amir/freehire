...
Boardly's UI as a standalone React app (Vite + React + TypeScript + plain CSS).
It talks to Boardly only through its JSON API (`/api/v1`) and shares no code
with the repository it currently sits in, so this folder can be moved to its own
repository as it is.

## Run it (development)

1. Start Boardly (it serves the API on its port, `8040` by default).
2. Here:
...
The page calls Boardly straight from the browser, so Boardly must list
this app's address in its `BOARDLY_CORS_ORIGINS` (a comma list), e.g.
...
|---|---|---|
| `API_BASE_URL` | Docker (`.env`) | Boardly's address **as the browser reaches it**, e.g. `http://<server>:8040` |
| `API_KEY` | Docker (`.env`) | Only when Boardly sets `BOARDLY_API_KEY` |
...
All of Boardly's pages, with the same flow and look (`src/styles/app.css` is a
