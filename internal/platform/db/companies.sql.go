...
WITH oj AS MATERIALIZED (
    ...
    WHERE closed_at IS NULL AND duplicate_of IS NULL AND company_slug <> ''
      ...
      AND ($1::bool OR category <> '' OR COALESCE(enrichment->>'category', '') NOT IN ('', 'other') OR is_tech IS TRUE)
),
counts AS (
...
domain_industry_map AS (
    ...
    JOIN unnest($3::text[]) WITH ORDINALITY AS i(industry, ord)
      ON i.ord = d.ord
),
derived_ind AS (
...
type RefreshCompanyFacetsParams struct {
	AcceptNonTech     bool     `json:"accept_non_tech"`
...
func (q *Queries) RefreshCompanyFacets(ctx context.Context, arg RefreshCompanyFacetsParams) (int64, error) {
	result, err := q.db.Exec(ctx, refreshCompanyFacets, arg.AcceptNonTech, arg.MappingDomains, arg.MappingIndustries)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
