...
- **Exactly two ways of reaching freehire, both narrow and explicit:**
  1. `exec.Command`-ing freehire's already-built worker binaries
     ...
     boardly-api's own container — never `docker exec` into a separate
  ...
  2. A **read-only** `database/sql` connection to freehire's own Postgres
     ...
     of a provider's boards are `active`/`pending`. **boardly-api's own
...
`replace` directive to `go.mod`, or has boardly-api write to Postgres is
...
- **Rebuilding the binary is not the same as rebuilding the deployment.**
  ...
  image (`docker-compose.yml`'s `boardly-api` service, built from the
  `boardly-api-build`/`boardly-api` stages in the root `Dockerfile`).
  A change isn't visible to anyone using the deployed tool until
  `docker compose up --build -d boardly-api` runs. If a change "doesn't
...
- **Subprocesses inherit boardly-api's environment**, `CATALOGUE_TECH_ONLY`
  included, so docker-compose must give boardly-api the SAME value as the
...
- **The API is the only interface, and it has no login.** `apiMiddleware`
  (`api.go`) wraps every route: `BOARDLY_API_KEY` set means a Bearer
  key is required, unset means open (local only); browsers are admitted by
  `BOARDLY_CORS_ORIGINS`, whose preflight the middleware answers. Every
...
```bash
docker compose up --build -d boardly-api   # rebuilds just this service
```
