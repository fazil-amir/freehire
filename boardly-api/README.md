...
boardly-api is a **separate Go module** (`boardly-api/go.mod`, its own
...
boardly-api has two, and only two, ways of reaching freehire, neither of
which is a code-level dependency:

1. Running freehire's already-built worker binaries — `bulk-add-boards`,
   `ingest`, `reindex` — as **local subprocesses inside boardly-api's own
   container**. Those binaries are copied into boardly-api's Docker
   image at build time from freehire's own build stage (see the
   `boardly-api-build` / `boardly-api` stages in the root
   `Dockerfile`) — they are not recompiled here, and boardly-api's own Go
...
2. A **read-only** `database/sql` connection to freehire's own Postgres
   ...
   provider's board already added" (see Data below). boardly-api's own
...
`data/combined_boards.csv` is boardly-api's own candidate list — moved
...
- `id` — a stable identifier per row (a short hash of
  ...
  that predates it.
- `added` — a legacy column, no longer written by boardly-api's own
...
each run their own boardly-api against the same repo checkout, so the
...
service — boardly-api's own Go code never writes to that database, ever.
...
the one-shot `boardly-api-init` service (docker-compose.yml) gives `data/`
the container's group with group write, before boardly-api starts, on
...
- **`BOARDLY_API_KEY`** — when set, every request needs
  ...
  network.
- **`BOARDLY_CORS_ORIGINS`** — the comma-separated browser origins
  allowed to call it (default `http://localhost:5173`, the UI's dev server).
  A server-to-server caller needs no entry.

## Running it

boardly-api is wired into the root `docker-compose.yml` as its own
...
(override the host port with `BOARDLY_HOST_PORT`). The root, `/`,
only answers where the API is. For the UI, run the separate web app and add
its address to `BOARDLY_CORS_ORIGINS`.
...
|---|---|---|---|
| Boardly API | http://localhost:8040/api/v1 | 8091 | `BOARDLY_HOST_PORT` |
...
8091 is deliberately NOT a host port: Beszel uses it. Boardly still
...
```bash
cd boardly-api
go build -o boardly-api .
DATA_DIR=./data PORT=8091 ./boardly-api
```
...
| `BOARDLY_API_KEY` | — | Required as `Authorization: Bearer <key>` when set; unset leaves the API open (see Access) |
| `BOARDLY_CORS_ORIGINS` | `http://localhost:5173` | Browser origins allowed to call the API, comma-separated |
...
- **Already-added providers only ever get crawled, never re-added** — the
  ...
  one.
- **System jobs** are Boardly's own daily chores: the **dead-board
...
- **Server stats** (`/system/stats`): the host's disk, memory and CPU (from
  ...
  `/system/df` and `/build`; boardly-api never holds the raw socket.
...
boardly-api's own binary has no Docker dependency — it runs
