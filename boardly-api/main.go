// Command boardly-api is a standalone internal ops service for managing
...
// solely to answer "is this already added" — boardly-api's own Go code
...
func main() {
	...
	// ./boardly-api/data -> /app/data) that already exists, but create it
	...
	mux.HandleFunc("GET /{$}", func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, http.StatusOK, map[string]string{"service": "boardly-api", "api": "/api/v1"})
	})

	log.Printf("boardly-api listening on :%s", port)
