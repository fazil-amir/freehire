...
// The JSON API — Boardly's only interface (/api/v1). A separate UI
...
//   - BOARDLY_API_KEY set: a request needs "Authorization: Bearer
//     <key>" (compared in constant time). Unset, the API is open — for local
//     development; never expose it publicly that way.
//   - BOARDLY_CORS_ORIGINS (comma list, default the Vite dev server,
...
func apiMiddleware(next http.HandlerFunc) http.HandlerFunc {
	key := os.Getenv("BOARDLY_API_KEY")
	origins := map[string]bool{}
	for _, o := range strings.Split(envOr("BOARDLY_CORS_ORIGINS", "http://localhost:5173"), ",") {
