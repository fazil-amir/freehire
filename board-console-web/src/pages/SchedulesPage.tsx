...
// then the added providers without a schedule, then Board Console's own
          ...
          {data && data.system.length > 0 && (
            <tbody className="plan-system">
              {shownSystem.length > 0 && (
                <tr className="plan-group">
                  <td colSpan={4}><span className="tl-tag">System</span><span className="plan-group-note">Board Console's own daily jobs — not a provider's crawls</span></td>
                </tr>
              )}
              {shownSystem.map((r) => {
                const key = "sys-" + r.key;
